...
export default {
  ...
  methods: {
    useMedia_check() {
      ...
      return typeof this.useMedia_values[index] !== "undefined"
        ? this.useMedia_values[index]
        : this.useMedia_defaultValue;
    },
    useMedia_set() {
  ...
  mounted() {
    if (typeof window !== undefined) {
      this.mediaQueryLists = this.useMedia_queries.map((q) =>
        window.matchMedia(q)
      );
  ...
  beforeDestroy() {
    if (typeof window !== undefined) {
      this.mediaQueryLists.forEach((mql) =>
        mql.removeListener(this.useMedia_set)
      );
    }
  },
};
